...
* UPDATE가 실행될 때 인덱스가 없다면 모든 레코드를 잠그게 된다.

### 5.3.3 레코드 수준의 잠금 확인 및 해제
* 5.1 버전부터는 레코드 잠금/대기 조회가 가능하고, 강제로 잠금 해제하려면 kill 하면 됨

## 5.4 MySQL의 격리 수준
트랜잭션의 격리 수준(isolation level) : 여러 트랜잭션이 통시에 처리될 때 특정 트랜잭션이 다른 트랜잭션에서 변경/조회하는 데이터를 볼 수 있게 허용하는 것

|                 | DIRTY READ | NON-REPEATABLE READ | PHANTOM READ  |                                 비고                                  | 
|:---------------:|:----------:|:-------------------:|:-------------:|:-------------------------------------------------------------------:|
| READ UNCOMMITED |     O      |          O          |       O       | 트랜잭션 변경 내용이 COMMIT, ROLLBACK 여부와 관계 없이 보이는 것. 정합성 문제 있으니 사용 안하는게... |
|  READ COMMITED  |     X      |          O          |       O       |         커밋된 결과를 가져옴. 트랜잭션 내에서 select가 여러 번 실행될 때, 부정합 발생 가능         |
| REPEATABLE READ |     X      |          X          | O(InnoDB는 없음) |        InnoDB 디폴트 격리수준. 바이너리 로그 가진 서버는 최소 repeatable read...        |
|  SERIALIZABLE   |     X      |          X          |       X       |                     가장 업격. 성능도 떨어짐. 읽기작업도 잠금 획득                     |

* Dirty Read : 트랜잭션 작업 완료되지 않아도 다른 트랜잭션에서 볼 수 있는 현상
* Repeatable Read : 같은 Select 쿼리 내에서는 같은 결과를 가져와야 한다.
* PHANTOM READ : 트랜잭션에서 수행한 변경 작업에 의해 레코드가 보였다 안보였다 하는 현상 -> select 하는 레코드에 쓰기 잠금을 걸어야 하는데 언두레코드에 잠금 걸 수 없음
